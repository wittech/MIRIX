...
# Mirix API style responses (intended to be easier to use vs getting true Message types)


class MirixMessage(BaseModel):
    """
    Base class for simplified Mirix message response type. This is intended to be used for developers who want the internal monologue, tool calls, and tool returns in a simplified format that does not include additional information other than the content and timestamp.
...
# Legacy Mirix API had an additional type "assistant_message" and the "function_call" was a formatted string

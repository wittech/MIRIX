...
from mirix.orm.cloud_file_mapping import CloudFileMapping as CloudFileMapping
...
class CloudFileMappingManager:
    ...
    def add_mapping(self, cloud_file_id, local_file_id, force_add=False):
        """
    ...
    def delete_mapping(self, cloud_file_id=None, local_file_id=None):
        ...
        with self.session_maker() as session:
            ...
            if cloud_file_id is not None:
                try:
                    mapping = CloudFileMapping.read(session, cloud_file=cloud_file_id)
            ...
            if local_file_id is not None:
                try:
                    mapping = CloudFileMapping.read(session, local_file=local_file_id)
                    mapping.hard_delete(session)
                except Exception:
                    pass

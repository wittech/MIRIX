...
class Agent(BaseAgent):
    def __init__(
        ...
        self.logger = logging.getLogger("Mirix")
    ...
    def execute_tool_and_persist_state(self, function_name: str, function_args: dict, target_mirix_tool: Tool):
        ...
        try:
            if target_mirix_tool.tool_type == ToolType.MIRIX_CORE:
                ...
                if function_name == 'search_in_memory':
                ...
                if function_name in ['core_memory_append', 'core_memory_replace']:
                    agent_state_copy = self.agent_state.__deepcopy__()
                    function_args["agent_state"] = agent_state_copy  # need to attach self to arg since it's dynamically linked
                if function_name in ['episodic_memory_insert', 'episodic_memory_replace']:
                ...
                if function_name in ['check_episodic_memory']:
                    function_args['timezone_str'] = self.user_manager.get_user_by_id(self.user.id).timezone
                ...
                annotations = get_function_annotations_from_source(target_mirix_tool.source_code, function_name)
    ...
    def _handle_ai_response(
        ...
        if response_message.function_call or (response_message.tool_calls is not None and len(response_message.tool_calls) > 0):
            ...
            # Failure Case 3: function name is OK, but function args are wrong
            if function_name == 'archival_memory_insert':
                if image_uris is None or 'image_uris' not in image_uris:
                    # TODO: now forcefully set it to None
                    function_args['image_indices'] = ''
                if "image_indices" in function_args and function_args['image_indices'] != '':
                    for index in function_args['image_indices'].split(","):
                        index = int(re.findall(r'\d+', index)[0])
                        if index > len(image_uris['image_uris']):
                            error_msg = f"Image index '{index}' out of range, there are only {len(image_uris['image_uris'])} images"
                            function_response = package_function_response(False, error_msg)
                            messages.append(
                                Message.dict_to_message(
                                    agent_id=self.agent_state.id,
            ...
            # Failure Case 5: function name is OK, but old_content is empty
            if function_name == 'core_memory_replace':
                if function_args['old_content'] == '' or len(function_args['old_content'].replace("\\n", "\n").split("\n")) > 10:
                    if function_args['old_content'] == '':
                        error_msg = f"Old content is empty. If you have nothing to replace, please use core_memory_append"
                    else:
                        error_msg = "Do not try to remove too much information at once, only edit a smaller snippet at one time."
                    function_response = package_function_response(False, error_msg)
                    messages.append(
                        Message.dict_to_message(
                            agent_id=self.agent_state.id,
            ...
            if function_name == 'trigger_memory_update':
                function_args["user_message"] = {'message': input_message, 
                                                 'image_uris': image_uris,
            ...
            heartbeat_request = function_args.pop("continue_chaining", None)
            ...
            if not isinstance(heartbeat_request, bool) or heartbeat_request is None:
                self.logger.warning(
                    f"{CLI_WARNING_PREFIX}'continue_chaining' arg parsed was not a bool or None, type={type(heartbeat_request)}, value={heartbeat_request}"
                )
            ...
            try:
                ...
                if function_name == 'core_memory_replace':
                    function_args['old_content'] = function_args['old_content'].strip()

                function_response = self.execute_tool_and_persist_state(function_name, function_args, target_mirix_tool)

                if function_name == 'core_memory_append' or function_name == 'core_memory_replace':
                    print(self.agent_state.memory.compile())

                if function_name == 'core_memory_replace':
                    if function_response is not None and "Error executing function core_memory_replace" in function_response:
                        function_response += '\n Try again.'
                        raise ValueError(function_response)

                if force_response and function_name != 'send_message':
                    heartbeat_request = True
    ...
    def step(
        ...
        **kwargs,
        ...
        first_input_messge = messages[0] if isinstance(messages, list) else messages
        counter = 0
        ...
        while True:
            ...
            if self.agent_state.name == 'meta_memory_agent' and step_count == 0:
                ...
                try:
                    ...
                    temporary_messages[0].text = temporary_messages[0].text.split("[System Message]")[0] + '\n' + \
                        "The above are the screenshots from the user, please look at these screenshots and extract the topic (brief description of what the user is focusing on) from these screenshots. If there are multiple focuses in these screenshots, then extract them all and put them into one string seperated by ';'. Call the function `update_topic` to update the topic with the extracted topics."
                    
                    functions = [{
                        'name': 'update_topic',
                        'description': "Update the topic of the screenshots. The topic will be used for retrieving relevant information from the database",
                        'parameters': {'type': 'string', 'properties': {'topic': {'type': 'string', 'description': 'The topic of the screenshots. If there are multiple topics then seperate them with ";".}'}}, 'required': ['topic']},
                    }]
    ...
    def inner_step(
        ...
        **kwargs,
        ...
        try:
            ...
            raw_system = in_context_messages[0].text

            if retrieved_memories is None:

                if topics is None:
                    key_words = self.agent_state.topic
                else:
                    key_words = topics

                timezone_str = self.user_manager.get_user_by_id(self.user.id).timezone

                # step 1: extract the core_memory
                current_persisted_memory = Memory(
                    blocks=[self.block_manager.get_block_by_id(block.id, actor=self.user) for block in self.agent_state.memory.get_blocks()]
                )
                core_memory = current_persisted_memory.compile()

                # step 2: extract the episodic_memory
                # TODO: make the size adjustable from the UI
                current_episodic_memory = self.episodic_memory_manager.list_episodic_events(agent_state=self.agent_state, limit=50, timezone_str=timezone_str)
                episodic_memory = ''
                if len(current_episodic_memory) > 0:
                    for idx, event in enumerate(current_episodic_memory):
                        episodic_memory += f"[Event ID: {event.id}] Timestamp: {event.occurred_at.strftime('%Y-%m-%d %H:%M:%S')} - {event.summary}\n"
                recent_episodic_memory = episodic_memory.strip()

                most_relevant_episodic_memory = self.episodic_memory_manager.list_episodic_events(agent_state=self.agent_state, query=key_words, search_field='summary', search_method='semantic_match', limit=50, timezone_str=timezone_str)
                ...
                if len(most_relevant_episodic_memory) > 0:
                    for idx, event in enumerate(most_relevant_episodic_memory):
                        most_relevant_episodic_memory_str += f"[Event ID: {event.id}] Timestamp: {event.occurred_at.strftime('%Y-%m-%d %H:%M:%S')} - {event.summary}\n"
                relevant_episodic_memory = most_relevant_episodic_memory_str.strip()

                # step 3: extract the resource memory
                current_resource_memory = self.resource_memory_manager.list_resources(agent_state=self.agent_state, query=key_words, search_field='summary', search_method='semantic_match', limit=50, timezone_str=timezone_str)
                resource_memory = ''
                ...
                # step 4: extract the procedural memory
                current_procedural_memory = self.procedural_memory_manager.list_procedures(agent_state=self.agent_state, query=key_words, search_field="description", search_method='semantic_match',limit=50, timezone_str=timezone_str)
                procedural_memory = ''
                if len(current_procedural_memory) > 0:
                    for idx, procedure in enumerate(current_procedural_memory):
                        procedural_memory += f"[{idx}] Entry Type: {procedure.entry_type}; Description: {procedure.description}\n"
                procedural_memory = procedural_memory.strip()

                # step 5: extract the semantic memory
                current_semantic_memory = self.semantic_memory_manager.list_semantic_items(agent_state=self.agent_state, query=key_words, search_field="concept", search_method='semantic_match',limit=50, timezone_str=timezone_str)
                semantic_memory = ''
                if len(current_semantic_memory) > 0:
                    for idx, semantic_memory_item in enumerate(current_semantic_memory):
                        semantic_memory += f"[{idx}] Concept: {semantic_memory_item.concept}; Definition: {semantic_memory_item.definition}\n"

                retrieved_memories = self.build_system_prompt(key_words, core_memory, recent_episodic_memory, relevant_episodic_memory, resource_memory, semantic_memory, procedural_memory)
                
            in_context_messages[0].text = raw_system + "\n\n" + retrieved_memories
            ...
            for response_choice in response.choices:
                ...
                tmp_response_messages, heartbeat_request, function_failed = self._handle_ai_response(
                    first_input_messge.text, # give the last message to the function so that other agents can see this message through funciton_calls
                    response_message,
                    image_uris=image_uris,
